...
void load_data(Seat outbound[], Seat inbound[]);
void save_data(Seat outbound[], Seat inbound[]);

void main_menu(Seat outbound[], Seat inbound[]);
void flight_menu(Seat flight[], const char *flight_name, Seat outbound[], Seat inbound[]);
...
void assign_seat(Seat flight[], int count, Seat outbound[], Seat inbound[]);
void delete_seat(Seat flight[], int count, Seat outbound[], Seat inbound[]);
...
void load_data(Seat outbound[], Seat inbound[]) {
    ...
    if (fread(&data, sizeof(ReservationData), 1, fp) == 1) {
        memcpy(outbound, data.outbound, sizeof(data.outbound));
        memcpy(inbound, data.inbound, sizeof(data.inbound));
    }
...
void save_data(Seat outbound[], Seat inbound[]) {
    ...
    memcpy(data.outbound, outbound, sizeof(data.outbound));
    memcpy(data.inbound, inbound, sizeof(data.inbound));
    ...
    fwrite(&data, sizeof(ReservationData), 1, fp);
    fclose(fp);
}
...
void main_menu(Seat outbound[], Seat inbound[]) {
    ...
    while (1) {
        ...
        switch (choice) {
            case 'a':
                flight_menu(outbound, "Outbound Flight", outbound, inbound);
                break;
            case 'b':
                flight_menu(inbound, "Inbound Flight", outbound, inbound);
                break;
            case 'c':
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 'c':
                ...
                break;
            case 'd':
                assign_seat(flight, TOTAL_SEATS, outbound, inbound);
                break;
            case 'e':
                delete_seat(flight, TOTAL_SEATS, outbound, inbound);
                break;
            case 'f':
                return;
            default:
